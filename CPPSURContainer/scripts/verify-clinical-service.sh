...
wait_gateway_route() {
  ...
  for _ in {1..40}; do
    status="$(curl --silent --output /dev/null --write-out '%{http_code}' "$route")"
    if [[ "$status" == "200" ]]; then
      return 0
    fi
    sleep 2
  ...
  fail "Gateway no encontró una instancia disponible de $description"
}
...
stage="esperar rutas del Gateway"
wait_gateway_route "http://localhost:8086/api/patients/search?q=87654321" "Pacientes"
wait_gateway_route "http://localhost:8086/api/scheduling/specialties" "Agenda"
wait_gateway_route "http://localhost:8086/api/billing/debts" "Facturación"
wait_gateway_route "http://localhost:8086/api/clinical/diagnoses/cie10?q=" "Clínica"

stage="crear paciente"; patient="$(curl --fail-with-body -sS -X POST -H 'Content-Type: application/json' --data '{"dni":"87654321","nombres":"Ana","apellidos":"Torres","fechaNacimiento":"1994-03-15","sexo":"F","telefono":"987654321","email":"ana@example.com","direccion":"Lima"}' http://localhost:8086/api/patients)" || fail "No se creó paciente"; patient_id="$(jq -r .id <<<"$patient")"
...
# Agenda expone las transiciones mediante /status; la ruta base es solo de consulta.
stage="llevar cita a consulta"; for state in EN_PISO EN_CONSULTA; do curl --fail-with-body -sS -X PATCH -H 'Content-Type: application/json' --data "{\"estado\":\"$state\"}" http://localhost:8086/api/scheduling/appointments/"$appointment_id"/status >/dev/null || fail "No se cambió a $state"; done
