...
public interface DeudaRepository extends JpaRepository<Deuda, Integer> {
    ...
    @Query("SELECT d FROM Deuda d LEFT JOIN FETCH d.paciente LEFT JOIN FETCH d.cita c LEFT JOIN FETCH c.especialidad LEFT JOIN FETCH c.ticket WHERE d.id = :id")
    Optional<Deuda> findByIdConDetalle(@Param("id") Integer id);
}
